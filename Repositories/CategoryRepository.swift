...
final class CategoryRepository {
    ...
    func add(name: String, icon: String = "folder") throws {
        try db.run(categories.insert(self.name <- name, self.icon <- icon))
    }
 
    func update(id: Int64, name: String, icon: String) throws {
        let row = categories.filter(self.id == id)
        try db.run(row.update(self.name <- name, self.icon <- icon))
    }
